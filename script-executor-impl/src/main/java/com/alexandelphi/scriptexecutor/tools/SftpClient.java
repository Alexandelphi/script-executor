package com.alexandelphi.scriptexecutor.tools;
...
public class SftpClient {
    ...
    public static void main(String[] args) {
        sendFileToServer("localhost");
    }

    public static void sendFileToServer(String ip) {
        SftpClient client = new SftpClient();
        client.setServer(ip);
        client.setPort(2210);
        client.setLogin("vagrant");
        client.setPassword("vagrant");

        String remoteDirectory = "/home/vagrant/Vagrantfile";
        String localDirectory = "tmp/Vagrantfile";

        client.connect();

        try {
            //client.uploadFile("src/main/resources/upload.txt", "/uploaded.txt");
            client.uploadFile(localDirectory, remoteDirectory);
